...
/**
 * `compareStrings`, `deepEqual` and `stableStringify` moved to `json.ts` —
 * they are JSON value utilities rather than schema ones. Re-exported here so
 * that every existing import keeps resolving.
 */
export { compareStrings, deepEqual, stableStringify } from './json.js';
